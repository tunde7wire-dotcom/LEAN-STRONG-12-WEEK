...
const oldChecklistLogic = `  const todayStr = getLocalTodayString();
...
// Escape backticks in old logic
let escapedOld = oldChecklistLogic.replace(/\\\`/g, '\`').replace(/\\\$/g, '$');

code = code.replace(escapedOld, newChecklistLogic);
